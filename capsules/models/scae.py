...
class ImageAutoencoder(Model):
    ...
    def __init__(
        ...
        self._primary_encoder = primary_encoder  # part encoder
        self._primary_decoder = primary_decoder  # part decoder
        self._encoder = encoder  # object encoder(transformer)
        self._decoder = decoder  # object decoder
        self._input_key = input_key  # 'image'
        self._label_key = label_key  # 'label'
        self._n_classes = n_classes  # 10  #10个类吧

        self._dynamic_l2_weight = dynamic_l2_weight # 10
        self._caps_ll_weight = caps_ll_weight # 1.
        self._vote_type = vote_type # 'enc'
        self._pres_type = pres_type # 'enc'
        self._img_summaries = img_summaries # 'False'

        self._stop_grad_caps_inpt = stop_grad_caps_inpt # True
        self._stop_grad_caps_target = stop_grad_caps_target # True
        self._prior_sparsity_loss_type = prior_sparsity_loss_type # 'l2'
    ...
    def _img(self, data, prep='none'):
        ...
        img = data[self._input_key]  # _input_key = 'image'
        if prep == 'sobel':
            img = preprocess.normalized_sobel_edges(img)  # 用上 sobel filter在上面
    ...
    def _build(self, data):
        ...
        input_pres = pres
        if self._stop_grad_caps_inpt:  # 'true'
            # 不理解这里的 stop_gradient在说什么
